...
public:
	...
	boost::shared_ptr<const T> read() const {
		return boost::atomic_load_explicit(&sp, boost::memory_order_consume);
	}
	...
	void reset(const boost::shared_ptr<T>& r) {
		boost::atomic_store_explicit(&sp, r, boost::memory_order_release);
	}

	void reset(boost::shared_ptr<T>&& r) {
		boost::atomic_store_explicit(&sp, std::move(r), boost::memory_order_release);
	}
	...
	void copy_update(R&& fun) {
		boost::shared_ptr<T> sp_l = boost::atomic_load_explicit(&sp, boost::memory_order_consume);
